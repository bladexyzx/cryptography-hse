...
class BlockCiphers:
    ...
    def recurrent_hill_cipher_encrypt(self, text, block_size, key_text_1, key_text_2):
            ...
            for i, block in enumerate(blocks):
                if i == 0:
                    ...
                    current_key = np.dot(key2, key1) % self.m
                    key1 = key2
                    key2 = current_key
                ...
                encrypted_block = [int(val) for val in C.flatten().tolist()]
                encrypted_blocks.extend(encrypted_block)
    ...
    def recurrent_hill_cipher_decrypt(self, encrypted_text, block_size, key_text_1, key_text_2):
                ...
                try:
                    det1_inv = pow(det1_int % self.m, -1, self.m)
                except ValueError:
                    raise KeyValidityError("Первый ключ не имеет обратного по модулю.")
                adj1 = np.round(np.linalg.inv(key1) * np.linalg.det(key1)).astype(int)
                key1 = (det1_inv * adj1) % self.m
                ...
                try:
                    det2_inv = pow(det2_int % self.m, -1, self.m)
                except ValueError:
                    raise KeyValidityError("Второй ключ не имеет обратного по модулю.")
                adj2 = np.round(np.linalg.inv(key2) * np.linalg.det(key2)).astype(int)
                key2 = (det2_inv * adj2) % self.m
                ...
                for i, block in enumerate(encrypted_blocks):
                    if i == 0:
                        ...
                        key_inv = np.dot(key1, key2) % self.m
                        key1 = key2
                        key2 = key_inv
